...
rows_butt_galleries = [
    gr.Button(
        LABEL_BUTT_GALLERY_OFF,
        value=LABEL_BUTT_GALLERY_OFF,
        scale=1,
        visible=True,
        elem_id=f"but-gallery_{i}",
    )
    for i in range(TAG_NUM_PER_PAGE)
]
rows_butt_galleries_notags = [
    gr.Button(
        LABEL_BUTT_GALLERY_OFF_NOTAGS,
        value=LABEL_BUTT_GALLERY_OFF_NOTAGS,
        scale=1,
        visible=True,
        elem_id=f"but-gallery_{i}",
    )
...
rows_payload = [
    gr.Textbox(
        value="", label="Payload", placeholder="empty", visible=True, interactive=True
    )
    for i in range(TAG_NUM_PER_PAGE)
]
...
def new_page_idx(page: int) -> None:
    return [
        gr.update(value=TAG_NUM_PER_PAGE * (page - 1) + i)
        for i in range(TAG_NUM_PER_PAGE)
    ]
...
def switch_gallery(switch: str, tag: str):
    ...
    ret_off = (
        gr.update(value=OFF),
        gr.update(type="pil", value=[img_empty], visible=False),
    )
...
def update_tags(page: int):
    ...
    for i, tag in enumerate(tags):
        ...
        update.append(
            gr.update(value=tag, label=f"Tag [size: {len(tags_summary['selection'])}]")
        )
...
def store_action(
    tag: str,
    action: Literal[
        "delete", "replace", "deselect", "deselect_notags", "no_action", "undef"
    ],
    payload: str,
):
    tag_summary: TagsSummary = pool.tags[tag]
...
with gr.Blocks() as interface:
    ...
    with gr.Row():
        ...
        page_select = gr.Dropdown(
            [1], label="Page", multiselect=False, value=1, scale=2, interactive=True
        )
        trigger = gr.Textbox(
            placeholder="Enter trigger tag",
            label="Trigger Tag",
            scale=2,
            interactive=True,
        )
    ...
    for i in range(TAG_NUM_PER_PAGE):
        ...
        rows_action[i].change(
            store_action, [rows_tag[i], rows_action[i], rows_payload[i]], None
        )

        rows_butt_galleries[i].click(
            switch_gallery,
            [rows_butt_galleries[i], rows_tag[i]],
            [rows_butt_galleries[i], rows_galleries[i]],
        )
        rows_butt_galleries_notags[i].click(
            switch_gallery_notags,
            [rows_butt_galleries_notags[i], rows_tag[i]],
            [rows_butt_galleries_notags[i], rows_galleries[i]],
        )

    pools_select.change(hide_groups, None, rows).then(flush_tags, None, rows_tag).then(
        new_pool, pools_select, None
    ).then(new_pool_page, pools_select, page_select).then(
        update_tags, page_select, rows_tag
    ).then(restore_actions, rows_tag_seq, rows_action).then(
        restore_payloads, rows_tag_seq, rows_payload
    ).then(show_groups, page_select, rows)
    page_select.change(update_tags, page_select, rows_tag).then(
        restore_actions, rows_tag_seq, rows_action
