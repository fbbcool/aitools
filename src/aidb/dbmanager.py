...
class DBManager:
    ...
    def get_hfd(self, repo_id: str | None) -> HFDatasetImg | None:
        """
        ...
        try:
            ...
            if config and isinstance(config, dict):
                ...
                if 'mongodb_settings' in config and isinstance(config['mongodb_settings'], dict):
                    ...
                    if 'collection' in mongo_settings and isinstance(
                ...
                if 'directory_settings' in config and isinstance(
                    config['directory_settings'], dict
                ):
                    directory_settings = config['directory_settings']
                    if 'root_dir' in directory_settings and isinstance(
                        directory_settings['root_dir'], str
                    ):
                        self._root = Path(directory_settings['root_dir'])

                # Load thumbnail settings
                if 'thumbnail_settings' in config and isinstance(
                    config['thumbnail_settings'], dict
                ):
                    thumb_settings = config['thumbnail_settings']
                    if 'default_thumbnail_dir' in thumb_settings and isinstance(
                        thumb_settings['default_thumbnail_dir'], str
                    ):
                        self._default_thumbnail_dir = (
                            self._root / thumb_settings['default_thumbnail_dir']
                        )
                    if (
                        'default_thumbnail_size' in thumb_settings
                        and isinstance(thumb_settings['default_thumbnail_size'], list)
                        and len(thumb_settings['default_thumbnail_size']) == 2
                    ):
                ...
                if 'train_image_settings' in config and isinstance(
                    config['train_image_settings'], dict
                ):
                    train_img_settings = config['train_image_settings']
                    if 'default_train_image_dir' in train_img_settings and isinstance(
                        train_img_settings['default_train_image_dir'], str
                    ):
                        self._default_train_image_dir = (
                            self._root / train_img_settings['default_train_image_dir']
                        )
                    if (
                        'default_train_image_size' in train_img_settings
                        and isinstance(train_img_settings['default_train_image_size'], list)
                        and len(train_img_settings['default_train_image_size']) == 2
                    ):
                ...
                if 'hf_settings' in config and isinstance(config['hf_settings'], dict):
                    ...
                    if 'collection2hfd' in hf_settings and isinstance(
                        hf_settings['collection2hfd'], list
                    ):
                        collection2hfd = {}
    ...
    def add_container(
        ...
        try:
            ...
            container_creation_date = (
                datetime.datetime.fromtimestamp(container_creation_timestamp).isoformat() + 'Z'
            )
            container_last_modified_date = (
        ...
        if recursive:
            ...
            for file_path_obj in container_path_obj.rglob('*'):
                if file_path_obj.is_file():
                    image_db_id = self._process_image_file(
                        file_path_obj,
                        container_path_obj,
                        image_extensions,
                        inserted_container_object_id,
                    )
                    if image_db_id:
    ...
    def _process_image_file(
        self,
        full_image_path_obj: Path,
        base_container_path_obj: Path,
        image_extensions: Dict[str, str],
        parent_container_db_id: ObjectId,
    ) -> Optional[str]:
        """
        ...
        if file_extension in image_extensions:
            ...
            try:
                ...
                file_creation_date = (
                    datetime.datetime.fromtimestamp(file_creation_timestamp).isoformat() + 'Z'
                )
            ...
            image_metadata = {
                ...
                'container_local_path': str(
                    base_container_path_obj
                ),  # This refers to the local path of the container
