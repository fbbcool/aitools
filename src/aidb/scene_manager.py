...
from ait.tools.files import is_img_or_vid, subdir_inc
...
class SceneManager:
    ...
    def url_update(self, url: str | Path) -> None:
        url = Path(url)
        if not url.exists():
    ...
    def scene_new(self, _urls: list[str] | list[Path] | str | Path) -> tuple[str, Path] | None:
