"""Legacy 1xlasm-specific captioning helpers.

Holds the trigger phrases, the xlasm-flavoured directive generator
(`xlasm_gen_directive`), forbidden-vocab + body-type-word + trigger-presence
validators, and the legacy `LABEL_PROMPT` / `CONTENT_PROMPT` dicts. These
predate the JSON-skin pipeline but are still consumed by:

- `pipeline.py` — standalone integrated-pipeline script.
- `script/img_caption.py` — standalone single-image captioning helper.
- `cell_scene_image.py:editor_labels` — legacy FIELD_LABELS UI.
- `test_skin.py` / `test_joy.py` — byte-equality / compose-equivalence tests.

The new pipeline (Joy / JoySceneDB / SkinRegistry) does NOT depend on this
file. New callers should use `skin.directive` / `skin.caption_violations`
/ `skin.missing_triggers` etc. instead. This module exists to keep the
legacy callers working without forcing them through the skin loader.
"""
import re
