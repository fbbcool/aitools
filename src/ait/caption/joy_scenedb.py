...
class JoySceneDB:
    ...
    def __init__(self, config: SceneConfig, trigger: str = '', verbose: int = 0):
    ...
    def _ids_caption(self, ids: list[str]) -> dict[str, str]:
        ...
        ret = {}
        for id in ids:
            caption = self._id_caption(id)
            if caption is None:
                continue
            ret |= {id: caption}
        return ret

    def _id_caption(self, id: str) -> Optional[str]:
        try:
        ...
        id_scene = scene_id_from_url(simg.url_from_data)
        if id_scene is None:
            return None
        ...
        hint = ''
        caption = self._joy.img_caption(img, labels=labels, hint=hint)
        return caption
