...
Hint sentinel: if the SceneImage's `hints` field is the literal string `'none'` (case-insensitive after `.strip()`), treat it as no hint — skip the `Preserve every verb…` line in the composed prompt. The runtime (`JoySceneDB`, `Skin.compile_user_prompt`) does the same normalization, so caption-time and prompt-compile-time stay in sync.
